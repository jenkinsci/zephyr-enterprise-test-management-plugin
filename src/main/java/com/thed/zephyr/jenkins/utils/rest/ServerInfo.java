...
public class ServerInfo {
	...
	public static boolean findServerAddressIsValidZephyrURL(String hostNameWithProtocol) {
		...
		HttpClientContext context = getClientContext(hostNameWithProtocol, "test.manager", "test.maager");
		...
		if (statusCode >= 200 && statusCode < 300) {
			...
			try {
				...
				if (string.contains(USER_NOT_FOUND_MSG)) status=true;
			} catch (ParseException e) {
				e.printStackTrace();
			} catch (IOException e) {
				e.printStackTrace();
	...
	public static Map<Boolean, String> validateCredentials(String zephyrURL,
		...
		HttpResponse response = null;
		try {
			String constructedURL = URL_GET_USERS.replace("{SERVER}", zephyrURL);
			...
			try {
				string = EntityUtils.toString(entity);
					...
					if (userObj.getString("username").trim().equals(username)) {
						String userRole = userObj.getJSONArray("roles").getJSONObject(0).getString("name").trim();
						
						if (userRole.equalsIgnoreCase("manager") || userRole.equalsIgnoreCase("lead")) {
							statusMap.clear();
				...
				if (!statusMap.containsKey(true)) {
					statusMap.clear();
					statusMap.put(true, INVALID_USER_ROLE);
